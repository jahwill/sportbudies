...
class DateFormatter {
  ...
  static String customDateFormatter(
    ...
    final DateFormat yearMonthDayFormat = DateFormat('yMd');
    final DateFormat readAbleLongDayFormat = DateFormat.yMMMMd();
    var initialDate = DateTime.now();
    ...
    if (dateFromAPI != initialDate) {
      if (formatStyle == 'dd-mm-yyyy') {
        //
        return dayMonthYearFormat.format(dateFromAPI).toString();
        //we would add more functionality format in future
      } else if (formatStyle == 'yMd') {
        //
        return yearMonthDayFormat.format(dateFromAPI).toString();
        //
      } else if (formatStyle == 'yMMMMd') {
        //
        return readAbleLongDayFormat.format(dateFromAPI).toString();
        //
      } else {
        //
        return dayMonthYearFormat.format(dateFromAPI).toString();
      ...
      //this happens if user fail to select anything
      if (formatStyle == 'dd-mm-yyyy') {
        return dayMonthYearFormat.format(DateTime.now()).toString();
      } else {
        return dayMonthYearFormat.format(DateTime.now()).toString();
      }
    }
